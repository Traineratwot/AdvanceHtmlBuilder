		...
		if (key == selWH) {
			...
			switch (mss[key].type) {
				...
				case "image":
					...
					break;
				default:
					TOOLS[mss[key].type].draw(mss[key])
					break;
			}
		}
	});
...
function mouseReleased() {
	if (mouseX > 0 && mouseY > 0 && mouseX < width && mouseY < height && dra) {
		if (tool == "hand") {
			...
			// m.settings.color = settings.color;
			if (m.sX > 0 && m.sY > 0) {
				mss.push(m);
			}
		}
...
function create() {
	...
	mss.forEach(function (val, i, arr) {
		...
		if (val.color != "#ffffff") {
			if (use_vars) {
				vars["color-" + i] = val.settings.color;
				style += "background-color:var(--color-" + i + ");";
			} else {
				style += "background-color:" + val.settings.color + ";";
			}
		}
		style += "}";
		style_data += style;
